...
namespace Valve.VR.InteractionSystem
{
	...
	public class Hand : MonoBehaviour
	{
		...
		private void UpdateHovering()
		{
			...
			float handDiff = Mathf.Abs( transform.position.y - playerInstance.trackingOriginTransform.position.y ); // The distance from the floor

            /*
             * @Note(colin): I commented this out cause it's shit
            float boxMultClamped = Util.RemapNumberClamped(handDiff, 0.0f, 0.5f * flHoverRadiusScale, 5.0f, 1.0f);
			float boxMult = boxMultClamped * flHoverRadiusScale;
            */
            ...
            Physics.OverlapBoxNonAlloc(
                hoverSphereTransform.position - new Vector3(0, flScaledSphereRadius - flScaledSphereRadius, 0),
                new Vector3(flScaledSphereRadius, flScaledSphereRadius * 2.0f, flScaledSphereRadius),
                overlappingColliders,
                Quaternion.identity,
                hoverLayerMask.value
            );
		...
		void OnDrawGizmos()
		{
            ...
            Gizmos.DrawWireSphere(sphereTransform.position, 1);
        }
